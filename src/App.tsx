...
const DrainageSVG = () => (
  <svg viewBox="0 0 100 100" className="w-full h-full rounded shadow-sm opacity-95">
    ...
    <rect x="25" y="0" width="50" height="100" fill="#292524" />
...
export default function App() {
  ...
  useEffect(() => {
    if (!isFirebaseConfigured || !auth) {
      ...
      try {
        ...
        setCellsData(saved ? JSON.parse(saved) : {});
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      docRef,
      (docSnap) => {
        setCellsData(docSnap.exists() ? (docSnap.data().cells || {}) : {});
        setLoading(false);
      },
      (err) => {
  ...
  const persistCells = useCallback(async (updatedCells: CellsData) => {
    ...
    if (isFirebaseConfigured && db && user && 'uid' in user && user.uid !== 'local-user') {
      ...
      try {
        ...
        try {
          localStorage.setItem(STORAGE_KEY, JSON.stringify(updatedCells));
        } catch { /* تجاهل */ }
      }
    } else {
      try {
  ...
  const handleCellClick = async (cellId: string) => {
    ...
    if (mode === 'edit') {
      ...
      const types: CellType[] = ['tree', 'water_canal', 'drainage', 'road'];
